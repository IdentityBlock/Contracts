...
contract Verifier{
    ...
    function verifyToken(string memory token, address user) public{
        verified_tokens[token] = user;
    }

    // get the wallet address of verified user by the token
    function getVerifiedUserAddress(string memory token) public view isOwner returns (address user){
